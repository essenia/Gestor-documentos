...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!/^\d+[smhd]$/.test(JWT_EXPIRES_IN)) {
      console.warn(
        `JWT_EXPIRES_IN inválido: ${JWT_EXPIRES_IN}, usando "8h" por defecto`
      );
      JWT_EXPIRES_IN = "8h";
    }
  //convertir el modelo a objeto plano con .toJSON() y luego usar la propiedad anidada (role.nombre).
    const userData = user.toJSON() as any;  // Convierte a objeto plano
const rol = userData.role?.nombre; 
    // const rol = (user as any)["role.nombre"];
